...
class EditProfileController extends GetxController implements GetxService {
  ...
  Future<void> getStudies() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (studiesList.isNotEmpty) {
        haveStudies = true;
        _selectedStudies = _studiesList.firstWhere((element) => element.id == selectedStudiesId);
  ...
  Future<void> getTermsByStudies(bool isNew) async {
    Response response = await parser.getTermsByStudiesId(selectedStudies.id as int);
    apiCalled = true;
    if (response.statusCode == 200) {
      Map<String, dynamic> myMap = Map<String, dynamic>.from(response.body);
      dynamic body = myMap["data"];
      ...
      if (_termsList.isNotEmpty && isNew == false) {
        haveTerms = true;
        _selectedTerms = _termsList.firstWhere((element) => element.id == selectedTermId);
  ...
  Future<void> register() async {
    if (firstName.text == '' || mobile.text == '' || lastName.text == '' || enrollment.text == '' || haveStudies == false || haveTerms == false || cover == '') {
    ...
    if (response.statusCode == 200) {
      parser.saveInfo(firstName.text, lastName.text, cover, mobile.text, selectedTerms.id as int, selectedStudies.id as int);
