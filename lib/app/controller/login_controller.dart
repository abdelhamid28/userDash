...
class LoginController extends GetxController implements GetxService {
  ...
  Future<void> login() async {
    ...
    var param = {"email": emailAddress.text, "password": password.text};
    ...
    if (response.statusCode == 200) {
      ...
      if (myMap['user'] != '' && myMap['token'] != '' && myMap['user']['type'] == 1) {
        if (myMap['user']['status'] == 1) {
          UserModel data = UserModel.fromJson(myMap['user']);
          parser.saveInfo(myMap['token'], data.id.toString(), data.firstName.toString(), data.lastName.toString(), data.email.toString(), data.cover.toString(), data.mobile.toString(),
              data.termId as int, data.studyId as int);
          var updateParam = {"id": data.id.toString(), 'fcm_token': parser.getFcmToken()};
